...
class Taskboard extends React.Component {
  ...
  fetchCards(filter) {
    ...
    return (
      fetchJSON(appURL(`/board/cards${buildQueryString(filter)}`)).then(board => {
  ...
  pollIssues = () => {
    ...
    const url = appURL(`/board/updates?cursor=${cursor}${buildQueryString(filter, '&')}`);
  ...
  moveIssue(id, column, before, after) {
    ...
    return fetchJSON(appURL('/board/issues/move'), {
      method: 'POST',
      body
    });
  }
